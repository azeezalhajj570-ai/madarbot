...
export function CampaignsPage({ account, onSaved }: { account: Agent; onSaved: (message: string) => void }) {
  ...
  async function loadCampaigns() {
    ...
    } catch { setCampaignsStatus('Failed to load campaigns') }
    finally { setLoadingCampaigns(false) }
  }

  useEffect(() => {
    const query = activeTab === 'quick-send' ? (bulkSourceGroupQuery || bulkTargetGroupQuery) : sendGroupQuery
    if (!query.trim()) { setGroups([]); return }
    const timer = setTimeout(() => {
      void agentsApi.fetchAgentGroups(account.id, query).then(setGroups).catch(() => setGroups([]))
    }, 350)
    return () => clearTimeout(timer)
  }, [account.id, activeTab, bulkSourceGroupQuery, bulkTargetGroupQuery, sendGroupQuery])

  useEffect(() => {
    void agentsApi.fetchAgentJobs(account.id, BULK_MESSAGE_TASK_KEY, 50)
      .then(setBroadcastJobs).catch(() => {})
  }, [account.id])

  useEffect(() => {
    if (!bulkSourceGroup?.tg_group_id) {
      setBulkMemberResults([]); setBulkMemberTotal(0); setBulkMemberStatus(null); setLoadingBulkMembers(false); return
    }
  ...
  async function loadSendLogs(campaignId: number, page = 1) {
    ...
    try {
      ...
      setSendLogs(result.items); setSendLogsTotal(result.total)
    } catch { setStatus('Failed to load send logs') }
    finally { setLoadingSendLogs(false) }
  }

  function resetCreateForm() { setCreateName(''); setCreateDescription(''); setCreateMessage(''); setStatus(null) }
  ...
  async function handleCreate() {
    ...
    setIsSaving(true)
    try {
      await agentsApi.createCampaign(account.id, { name: createName.trim(), description: createDescription.trim() || undefined, message_template: createMessage.trim() })
      resetCreateForm(); setShowCreateForm(false); onSaved('Campaign created'); await loadCampaigns()
  ...
  async function viewCampaign(campaign: Campaign) {
    try {
      ...
      setDetailCampaign(c); setSendLogs([]); setSendLogsTotal(0); setSendLogsPage(1); setShowSendPicker(false); setSendResult(null)
      if (c.status !== 'draft') await loadSendLogs(c.id)
    ...
    if (bulkSummary) {
      ...
      try {
        const jobPayload: Record<string, unknown> = {
          target_type: bulkTargetType, message: bulkMessage.trim(), threshold, interval_seconds: intervalSeconds,
        }
            ...
            {sendLogs.map((log) => (
              <div key={log.id} style={{ padding: 8, borderRadius: 8, border: '1px solid var(--miniapp-border-soft)', background: 'var(--miniapp-surface)', fontSize: 12, display: 'grid', gap: 2 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ background: log.status === 'sent' ? 'var(--miniapp-sage-dim)' : log.status === 'failed' ? 'rgba(161,87,62,0.12)' : 'var(--miniapp-bg-deep)', padding: '1px 6px', borderRadius: 4, fontSize: 10, fontWeight: 600,
  ...
  return (
    <>
      <div style={{ display: 'flex', gap: 4, padding: 4, marginBottom: 12, background: 'var(--miniapp-bg)', borderRadius: 10, border: '1px solid var(--miniapp-border-soft)' }}>
        {(['campaigns', 'quick-send'] as const).map((tab) => (
          <button key={tab} type="button" onClick={() => setActiveTab(tab)} style={{
            flex: 1, padding: '8px 12px', border: 'none', borderRadius: 8, cursor: 'pointer',
            background: activeTab === tab ? 'var(--miniapp-surface)' : 'transparent',
            color: activeTab === tab ? 'var(--miniapp-text-primary)' : 'var(--miniapp-text-muted)',
            fontWeight: activeTab === tab ? 600 : 400, fontSize: 13, fontFamily: 'var(--miniapp-sans)',
          }}>{tab === 'campaigns' ? 'Campaigns' : 'Quick Send'}</button>
        ))}
      </div>
      ...
      {activeTab === 'campaigns' ? (
        <>
          <Card title="Campaigns" subtitle="Create a campaign once, send to any group anytime.">
            ...
            {!showCreateForm ? <Button onClick={() => { resetCreateForm(); setShowCreateForm(true) }}>New Campaign</Button> : null}
            {showCreateForm ? (
                ...
                {campaigns.map((c) => (
                  <div key={c.id} onClick={() => viewCampaign(c)} style={{ cursor: 'pointer', padding: 10, borderRadius: 10, border: '1px solid var(--miniapp-border-soft)', background: 'var(--miniapp-surface)', display: 'grid', gap: 4 }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--miniapp-text-muted)', display: 'flex', gap: 12 }}>
                      <span>Sent: {c.sent_count}</span><span>Failed: {c.failed_count}</span><span>Skipped: {c.skipped_count}</span>
                    </div>
        ...
        <>
          <Card title="Quick Send" subtitle="Select a campaign or send without one.">
            {status ? <Note>{status}</Note> : null}
            ...
            <div style={{ display: 'flex', gap: 4, padding: 4, background: 'var(--miniapp-bg)', borderRadius: 10, border: '1px solid var(--miniapp-border-soft)' }}>
              {(['members', 'groups'] as const).map((t) => (
                <button key={t} type="button" onClick={() => { setBulkTargetType(t); setBulkSummary(null) }} style={{
                  ...
                  fontWeight: bulkTargetType === t ? 600 : 400, fontSize: 13, fontFamily: 'var(--miniapp-sans)',
              ...
              <>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                  {bulkSelectedMembers.length ? (
                    <button type="button" onClick={() => setBulkSelectedMembers([])} style={{ background: 'var(--miniapp-bg)', color: 'var(--miniapp-text-primary)', border: '1px solid var(--miniapp-border-soft)', borderRadius: 12, padding: '8px 10px', fontSize: 16, lineHeight: '18px', cursor: 'pointer' }}>✕ {bulkSelectedMembers.length}</button>
                  ) : null}
            ...
            <div style={{ display: 'flex', gap: 4, padding: 4, background: 'var(--miniapp-bg)', borderRadius: 10, border: '1px solid var(--miniapp-border-soft)' }}>
              {(['now', 'schedule'] as const).map((m) => (
                <button key={m} type="button" onClick={() => setBulkScheduleMode(m)} style={{
                  ...
                  fontWeight: bulkScheduleMode === m ? 600 : 400, fontSize: 13, fontFamily: 'var(--miniapp-sans)',
            ...
            {bulkSummary ? (
              <div style={{ display: 'grid', gap: 8, padding: 12, border: '1px solid var(--miniapp-border-soft)', borderRadius: 12, background: 'var(--miniapp-bg)', fontSize: 13 }}>
                ...
                {bulkSummary.final_count === 0 ? <div style={{ color: 'var(--miniapp-coral)', fontSize: 13 }}>No recipients left.</div> : null}
              </div>
            ) : null}
            ...
            <Card title="Recent Jobs" subtitle="Recent broadcast and scheduled messages.">
              <div style={{ display: 'grid', gap: 6 }}>
                {broadcastJobs.map((job) => {
                  const p = job.progress || {}; const total = p.total_count ?? 0; const sent = p.success_count ?? 0; const failed = p.failure_count ?? 0
                  return (
