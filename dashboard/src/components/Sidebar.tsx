  ...
  return (
    ...
    >
      ...
      <div style={{ padding: `${spacing.md}px ${spacing.lg}px`, borderTop: `1px solid ${uiVars.border}` }}>
        ...
        <button onClick={async () => { try { await logoutUser() } catch {} clearAuth(); sessionStorage.setItem('just_logged_out', '1'); window.location.href = '/dashboard/login' }} style={{ background: 'none', border: 'none', padding: 0, display: 'flex', alignItems: 'center', gap: 6, color: uiVars.textSubtle, cursor: 'pointer', fontWeight: 500, fontSize: 12, marginTop: 2 }}>
          <LogOut size={10} />
          {t('sidebar.logout')}
        </button>
      </div>
