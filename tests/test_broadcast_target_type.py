...
class TestNormalizeGroupMemberBroadcastPayload:
    ...
    def test_selected_user_ids_validation(self):
        with pytest.raises(ValueError, match="selected_user_ids must contain valid integer user ids"):
            normalize_group_member_broadcast_payload({
                "target_type": "members",
                "source_group_id": -100123,
                "message": "Hi",
    ...
    def test_target_group_ids_validation(self):
        with pytest.raises(ValueError, match="target_group_ids must contain valid integer group ids"):
            normalize_group_member_broadcast_payload({
                "target_type": "groups",
