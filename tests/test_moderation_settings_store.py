...
async def test_moderation_settings_store_applies_defaults_and_typed_values(
    db_session, seeded_group
) -> None:
    db_session.add(
        GroupSetting(group_id=seeded_group["group_id"], key="anti_spam", value={"value": False})
    )
    db_session.add(
        GroupSetting(
            group_id=seeded_group["group_id"], key="anti_ads_mute_limit", value={"value": "4"}
        )
    )
    db_session.add(
        GroupSetting(group_id=seeded_group["group_id"], key="warn_limit", value={"value": 6})
    )
...
async def test_feature_enabled_guard_uses_moderation_settings_store(
    db_session, seeded_group
) -> None:
    db_session.add(
        GroupSetting(group_id=seeded_group["group_id"], key="anti_spam", value={"value": False})
...
async def test_settings_service_unwrap_value_supports_internal_views(
    db_session, seeded_group
) -> None:
    db_session.add(
        GroupSetting(
    ...
    assert ModerationSettingsStore(db_session).settings.unwrap_value(stored) == {
        "destination": "@alerts"
    }


async def test_settings_service_typed_adapters_parse_wrapped_legacy_values(
    db_session, seeded_group
) -> None:
    db_session.add(
        GroupSetting(group_id=seeded_group["group_id"], key="anti_bots", value={"value": "false"})
    )
    db_session.add(
        GroupSetting(group_id=seeded_group["group_id"], key="warn_limit", value={"value": "7"})
    )
    await db_session.commit()
