...
async def test_heuristic_english_investment_scam():
    ...
    decision = await classifier.classify(
        "Get guaranteed profit daily! Double your money in 24 hours!"
    )
...
async def test_heuristic_shortened_link_detection():
    ...
    )  # Confidence increases for multiple signals if I implement it that way
...
async def test_moderation_service_idempotency(db_session, fake_bot):
    ...
    stmt = select(ModerationEvent).where(
        ModerationEvent.group_id == group_id, ModerationEvent.message_id == 100
    )
