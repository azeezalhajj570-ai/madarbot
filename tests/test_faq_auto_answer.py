...
from bot.db.models.faq import FAQMode, FAQInteractionStatus
...
async def test_faq_service_flow(db_session: AsyncSession):
    ...
    # 1. Setup settings
    await service.get_settings(group_id)
    await service.update_settings(
        group_id, enabled=True, safe_mode=False, default_mode=FAQMode.AUTO_REPLY
    )
...
async def test_safe_mode_downgrade(db_session: AsyncSession):
    ...
    await service.update_settings(
        group_id=group_id, enabled=True, safe_mode=True, default_mode=FAQMode.AUTO_REPLY
    )

    await service.add_entry(group_id=group_id, question="Is it safe?", answer="Yes it is.")
