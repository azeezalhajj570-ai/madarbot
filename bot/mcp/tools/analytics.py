...
def register_analytics_tools(server: FastMCP) -> None:
    ...
    async def madarbot_get_analytics(agent_id: int | None = None) -> str:
        ...
        async with SessionLocal() as session:
            ...
            active = len([a for a in agents if a.auth_state == "active"])
            agent_summaries = [
                {
                    "id": a.id,
            ...
            result = success_response(
                content=f"{len(agents)} agent{'s' if len(agents) != 1 else ''} linked, {active} active",
                data={
        ...
        async with SessionLocal() as session:
            ...
            if agent is None:
                result = error_response(
                    ...
                    code="NOT_FOUND",
                    message=f"No agent found with agent_id={agent_id}",
                )
                return to_mcp_text(result)
            if int(agent.linked_by_user_id) != ctx.actor_user_id:
                result = error_response(
                    ...
                    code="ACCESS_DENIED",
                    message="You do not have permission to view this agent's settings",
                )
                return to_mcp_text(result)
            result = success_response(
                content=f"Safety settings for agent {agent.id}",
                data={
                    "agent_id": agent.id,
    ...
    async def madarbot_update_safety_settings(
        ...
        if safety_mode_enabled is False:
            result = error_response(
                content="Safety mode cannot be disabled",
                code="VALIDATION_ERROR",
                message="Safety mode is required and cannot be turned off",
            )
            return to_mcp_text(result)

        if max_actions_per_hour and max_actions_per_hour > _MAX_ACTIONS_PER_HOUR:
            result = error_response(
                content="max_actions_per_hour too high",
                code="VALIDATION_ERROR",
                message=f"max_actions_per_hour cannot exceed {_MAX_ACTIONS_PER_HOUR}",
            )
            return to_mcp_text(result)

        if max_messages_per_day and max_messages_per_day > _MAX_MESSAGES_PER_DAY:
            result = error_response(
                content="max_messages_per_day too high",
                code="VALIDATION_ERROR",
                message=f"max_messages_per_day cannot exceed {_MAX_MESSAGES_PER_DAY}",
            )
            return to_mcp_text(result)

        if min_delay_seconds is not None and min_delay_seconds < _MIN_DELAY_SECONDS:
            result = error_response(
                content="min_delay_seconds too low",
                code="VALIDATION_ERROR",
                message=f"min_delay_seconds cannot be less than {_MIN_DELAY_SECONDS}",
            )
            return to_mcp_text(result)

        if cooldown_minutes is not None and cooldown_minutes < _MIN_COOLDOWN_MINUTES:
            result = error_response(
                content="cooldown_minutes too low",
                code="VALIDATION_ERROR",
                message=f"cooldown_minutes cannot be less than {_MIN_COOLDOWN_MINUTES}",
            )
            return to_mcp_text(result)
        ...
        async with SessionLocal() as session:
            ...
            if int(agent.linked_by_user_id) != ctx.actor_user_id:
                result = error_response(
                    content="Access denied",
                    code="ACCESS_DENIED",
                    message="You do not have permission to update this agent's settings",
                )
                return to_mcp_text(result)
            ...
            result = success_response(
                content="Safety settings updated successfully",
                data={
                    "agent_id": agent.id,
                    "safety_mode_enabled": agent.safety_mode_enabled,
                    "max_actions_per_hour": agent.max_actions_per_hour,
