    ...
    if standalone:
        ...
        for msg in standalone:
            if not current_group:
                ...
                same_sender = (
                    last.get("sender_user_id") == msg.get("sender_user_id")
                    and msg.get("sender_user_id") is not None
                )
                if (
                    curr_date
                    and last_date
                    and (curr_date - last_date) <= timedelta(minutes=CONVERSATION_IDLE_MINUTES)
                    and same_sender
                ):
                    current_group.append(msg)
                else:
                    created += await _save_time_group(
