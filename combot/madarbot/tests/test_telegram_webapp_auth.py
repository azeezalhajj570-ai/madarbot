...
def test_validate_init_data_rejects_malformed_init_data() -> None:
    with pytest.raises(TelegramWebAppAuthError, match="Malformed"):
        validate_init_data("test", bot_token="123456:TESTTOKEN")


def test_validate_init_data_rejects_missing_hash() -> None:
    with pytest.raises(TelegramWebAppAuthError, match="Missing hash"):
        validate_init_data("auth_date=123&user=%7B%7D", bot_token="123456:TESTTOKEN")


def test_auth_endpoint_returns_401_on_malformed_init_data(api_client, db_session) -> None:
    response = api_client.get(
        "/webapp/tasks/catalog",
        headers={"X-Telegram-Init-Data": "test", "X-App-Boundary": "agents"},
    )
