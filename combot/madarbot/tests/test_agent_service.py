...
def _fake_incr(store: dict, key: str) -> int:
...
@pytest.mark.asyncio
async def test_agent_service_validates_group_member_broadcast_job_payload(db_session, fake_redis) -> None:
    ...
    with patch("redis.asyncio.Redis.from_url", return_value=fake_redis):
        fake_redis.incr = AsyncMock(side_effect=lambda key: _fake_incr(fake_redis._store, key))
