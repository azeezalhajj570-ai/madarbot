        ...
        try:
            ...
            async for dialog in managed_client.iter_dialogs():
                ...
                group = await entity_resolver.get_or_create_scraped_group(
                    ...
                    title=str(getattr(entity, "title", None)) if getattr(entity, "title", None) else None,
                    username=str(getattr(entity, "username", None)) if getattr(entity, "username", None) else None,
                    group_type="channel" if getattr(entity, "broadcast", False) else "supergroup" if getattr(entity, "megagroup", False) else "group",
                    member_count=int(getattr(entity, "participants_count", None) or getattr(entity, "user_count", None) or 0) or None,
                    raw_data={"id": getattr(entity, "id", None), "access_hash": getattr(entity, "access_hash", None)},
                    commit=True,
