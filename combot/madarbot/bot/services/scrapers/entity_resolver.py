...
async def get_or_create_scraped_group(
    *,
    ...
    if not commit:
        ...
        if group is None:
            group = ScrapedGroup(
                tg_group_id=canonical_id, last_agent_id=last_agent_id,
                title=title, username=username, group_type=group_type,
                member_count=member_count, description=description,
                raw_data=raw, created_at=now, updated_at=now,
            )
            session.add(group)
            return group
        ...
        if raw:
            merged = dict(group.raw_data or {})
            merged.update(raw)
            group.raw_data = merged
    ...
    update_cols = {k: values[k] for k in (
        "last_agent_id", "title", "username", "group_type",
        "member_count", "description", "raw_data", "updated_at",
    ) if values[k] is not None}

    stmt = (
        pg_insert(ScrapedGroup)
        .values(**values)
        .on_conflict_do_update(
            index_elements=["tg_group_id"],
            set_=update_cols,
            where=(ScrapedGroup.tg_group_id == canonical_id),
        )
        .returning(ScrapedGroup)
    )
