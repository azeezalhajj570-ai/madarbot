...
class Plan(Base):
    ...
    __tablename__ = "plans"
    __table_args__ = (
        UniqueConstraint("product_id", "slug", name="uq_plan_product_slug"),
    )
...
class Subscription(Base):
    ...
    promo_code_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("promotion_codes.id", ondelete="SET NULL"), nullable=True,
    )
...
class SubscriptionItem(Base):
    ...
    plan_price_id: Mapped[Optional[int]] = mapped_column(ForeignKey("plan_prices.id", ondelete="SET NULL"), nullable=True)
...
class Payment(Base):
    ...
    tenant_id: Mapped[int] = mapped_column(ForeignKey("tenants.id", ondelete="CASCADE"), index=True)
    provider: Mapped[str] = mapped_column(String(32), nullable=False)  # stripe, manual, telegram_stars
