...
class AgentListenerManager:
    ...
    async def _get_dedup_redis(self):
        if self._dedup_redis is not None:
            return self._dedup_redis
        from redis.asyncio import Redis
        try:
            self._dedup_redis = Redis.from_url(get_settings().redis_url, decode_responses=True)
        except Exception:
            return None
        return self._dedup_redis
    ...
    async def _handle_telethon_message(self, agent_id: int, event: Any) -> None:
        ...
        if message_id is not None and chat_id < 0:
            dedup_key = f"proc:msg:{agent_id}:{chat_id}:{message_id}"
            try:
                redis = await self._get_dedup_redis()
                if redis is not None:
                    if not await redis.set(dedup_key, "1", ex=3600, nx=True):
                        return
            except Exception:
