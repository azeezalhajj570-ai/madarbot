...
function AccountTasksPage({ account, onSaved }: { account: Agent; onSaved: (message: string) => void }) {
  ...
  async function refresh() {
    setLoading(true)
    try {
      const nextCatalog = await agentsApi.fetchTaskCatalog()
      setCatalog(nextCatalog)
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'Failed to load task catalog')
    }
    if (account.group_id) {
      try {
        const nextTasks = await agentsApi.fetchGroupTasks(account.group_id)
        setTasks(nextTasks)
      } catch (error) {
        setStatus(error instanceof Error ? error.message : 'Failed to load group tasks')
      }
    }
    setLoading(false)
  }
